...
export function AuthProvider({ children }) {
  ...
  // Global "you need to sign in to do that" prompt (2026-08-29, per Ryan:
  // clicking the favorite heart while signed out should pop open the
  // existing Sign In/Register box rather than silently doing nothing —
  // shown a toast-only option, a hover tooltip, and this before picking
  // this one). Lives here rather than in Nav.js (which owns its own
  // inline Sign In/Register dropdown) because the trigger can be anywhere
  // on the page — a listing card's heart icon, potentially far from the
  // nav and scrolled out of view — so it needs to be a page-level modal,
  // not a dropdown anchored to a nav button. `message` is set by whoever
  // calls promptSignIn() so the popup can say why it appeared (e.g. "Sign
  // in to save this property to your favorites"). Rendered by
  // AuthPromptHost (app/layout.js), which is what actually reads this
  // state back out — kept out of this file to avoid a circular import
  // (AuthPromptModal renders AuthPanel, which itself calls useAuth()).
  const [authPrompt, setAuthPrompt] = useState({ open: false, message: '' });
  const promptSignIn = useCallback((message) => setAuthPrompt({ open: true, message: message || '' }), []);
  const closeAuthPrompt = useCallback(() => setAuthPrompt({ open: false, message: '' }), []);
  ...
  const value = useMemo(
    ...
    [token, user, ready, login, register, signOut, updateUser, authPrompt, promptSignIn, closeAuthPrompt]
  );
