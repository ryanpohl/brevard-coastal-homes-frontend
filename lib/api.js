...
// Finding #2's pattern: a monthly rental record synced alongside its
// matching sale record at the identical address, priced far below any
// genuine sale in this market. Every rental example found in the report was
// priced at $10,000 or less, against sale prices of $165,000+ for the same
// addresses; Finding #4 separately notes the market median runs well into
// six figures. $12,000 leaves margin above the highest rental price seen
// and well below the lowest genuine sale price seen. Land is excluded
// because vacant land can legitimately be priced that low. This is a
// heuristic inferred from the report's examples, not a real "this is a
// rental" flag from the feed — a genuinely distressed, very-low-priced
// Home/Condo sale could in theory also get caught by it.
const RENTAL_PRICE_CEILING = 12000;
function looksLikeRentalArtifact(listing) {
  return (
    (listing.propertyType === 'Home' || listing.propertyType === 'Condo') &&
    listing.price != null &&
    listing.price < RENTAL_PRICE_CEILING
  );
}
