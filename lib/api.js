 ...
 *   priceMin, priceMax, beds, baths, waterfront, zoning, mlsNumber, address,
 *   sort, page, pageSize (see backend/README.md for the full list). Array
 *   values are comma-joined. mlsNumber/address power the hero search box's
 *   "Address, City, or MLS Number" field (see app/search/page.js).
...
// Address typeahead for the hero search box (2026-09-13, per Ryan: "Is
// there a way to have the search prepopulate as you are entering in an
// address?"). Hits the backend's dedicated /suggest endpoint (a fast
// prefix match, not the full /?address= substring search getListings
// above uses) — see SearchBar.js for the debounced caller and
// listings.controller.js's suggestAddresses for the matching backend half.
// `revalidate: false` (-> cache: 'no-store') since this fires on every
// keystroke and Next's fetch cache would otherwise happily "cache" a
// query string no one will ever type again.
export function getAddressSuggestions(q) {
  return request(`/api/listings/suggest?q=${encodeURIComponent(q)}`, { revalidate: false });
}
