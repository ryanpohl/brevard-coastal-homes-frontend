...
// Atlin Cove moved from its original alphabetical spot (1st) to last
// (2026-09-15, per Ryan: "Can you put the Atlin Cove link last on the pages
// since it doesn't have any listings yet") — it's the one comingSoon:true
// entry with no active inventory, so it's de-prioritized behind the 5
// communities that actually have (or can have) listings. This single array
// is the one source of truth both link lists render from in-order (no
// separate sorting logic in page.js): the hub page's 2-column/3-row grid
// (app/neighborhoods/[slug]/page.js's isVieraBuildersCommunitiesVieraWest
// block, which just .map()s this array) and each individual sub-community
// page's "See also" sibling-links line (same file's subCommunity block,
// which .filter()s this array but keeps its order) — so reordering it here
// alone moves Atlin Cove to last in both places, everywhere this array is
// read.
