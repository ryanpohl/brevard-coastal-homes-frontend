...
{/* LEFT: photos, header, stats, description, map — widened by 1in
            (96px) at the scheduling/contact panel's expense, per Ryan
            (2026-08-10). flex-basis is shifted +/-96px off the 50/50 split
            (rather than a plain width change) so the ratio holds responsively
            instead of only at one viewport size; minWidth keeps the old
            320px floor so narrow screens still stack to one column, same as
            the previous grid's auto-fit/minmax behavior. */}
      <div style={{ flex: '1 1 calc(50% + 96px)', minWidth: 320 }}>
...
{/* RIGHT: contact panel (Call/Text + Make an Offer + Ask a Question +
            inline Request Showing) — narrowed by 1in (96px), see the LEFT
            column comment above for why this uses flex-basis instead of a
            plain width. */}
      <div style={{ flex: '1 1 calc(50% - 96px)', minWidth: 320 }}>
